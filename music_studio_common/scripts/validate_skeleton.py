...
def main() -> int:
    ...
    if isinstance(core_motif, list) and core_motif:
        ...
        if task.get("motifMatchMode") == "ordered-subsequence":
            motif_index = 0
            for letter in melody_letters:
                if motif_index < len(motif_letters) and letter == motif_letters[motif_index]:
                    motif_index += 1
            inherited = motif_index == len(motif_letters)
        else:
            inherited = any(
        ...
        if not inherited:
            fail(errors, f"主旋律没有按顺序继承核心身份音：{'-'.join(motif_letters)}。允许加入连接音，但不能改变身份音顺序。")

    cadence_degree = task.get("cadenceDegree")
    if cadence_degree in {1, 3, 5} and melody:
        try:
            last_note = max(melody, key=lambda note: float(note["beat"]) + float(note["duration"]))
            match = PITCH_RE.match(str(last_note.get("pitch", "")))
            expected_letter = DEGREE_TO_LETTER[cadence_degree]
            if not match or match.group(1) != expected_letter:
                fail(errors, f"主旋律结尾应落在设计卡指定的 {cadence_degree} 音级（{expected_letter}），实际为 {last_note.get('pitch')}。")
        except (KeyError, TypeError, ValueError):
            pass
