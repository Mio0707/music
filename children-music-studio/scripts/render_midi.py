...
def main() -> int:
    ...
    drum_notes = [
        {
            ...
            "drum": item["instrument"],
            "velocity": int(item.get("velocity", 92)),
        }
    ...
    for note in drum_notes:
        ...
        events.extend([
            (start, DRUMS[note["drum"]], True, note["velocity"]),
            (end, DRUMS[note["drum"]], False, 0),
        ])
    events.sort(key=lambda item: (item[0], item[2]))
    last_tick = 0
    for tick, value, is_on, velocity in events:
        drum_track.append(Message("note_on" if is_on else "note_off", note=value, velocity=velocity, channel=9, time=tick - last_tick))
        last_tick = tick
